...
import { WikiArticle, WikiPage, UserProfile, ArticleHistoryItem } from '../types';
...
export const ArticleViewer: React.FC<ArticleViewerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'article' | 'source' | 'history' | 'info'>('article');
  ...
  const handleRestore = (item: ArticleHistoryItem) => {
    if (onRestoreRevision) {
      ...
      // Fallback: Open in editor with restored text
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className={`p-1 rounded border transition ${
              activeTab === 'history'
                ? 'bg-blue-50 dark:bg-blue-950 border-blue-400 text-blue-600 dark:text-blue-300'
